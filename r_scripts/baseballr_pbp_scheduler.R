...
query <- "SELECT DISTINCT processdate FROM raw.mlb_pbp_data;"
...
for (current_date in date_sequence) {
  ...
  pbp_data_table_name <- DBI::Id(schema = "raw", table = "mlb_pbp_data")
  ...
  people_data_table_name <- DBI::Id(schema = "raw", table = "mlb_people_data")
  # Write to PostgreSQL table (append mode)
  dbWriteTable(
    con,
  ...
  team_data_table_name <- DBI::Id(schema = "raw", table = "mlb_team_data")
  # Write to PostgreSQL table (append mode)
  dbWriteTable(
    con,
